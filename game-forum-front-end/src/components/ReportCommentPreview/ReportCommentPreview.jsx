...
function ReportCommentPreview({comment, resetData}) {
  ...
  const handleDeleteReport = async (e) => {
    if (window.confirm("Are you sure you want to delete this report?")) {
      const result = await deleteReport(userHeaders, comment['report_details']['id']);
      if (result) {
  ...
  const handleArchiveReport = async (e) => {
    if (window.confirm("Are you sure you want to archive this report?")) {
      const result = await archiveReport(userHeaders, comment['report_details']['id']);
      if (result) {
  ...
  return (
    ...
    >
      <div className="text-left">
        <h6 className="text-sm">{capitalize(comment['report_details']['report_reason'])} Report By : {comment['reporter_details']['username']}</h6>
        <h4 onClick={() => navigate(`/posts/${comment['post_details']['id']}`)} className="font-semibold cursor-pointer text-2xl hover:underline hover:font-bold">{comment['comment_details']['body']}</h4>
        <h6 className="text-sm">Posted By : {comment['owner_details']['username']}</h6>
      </div>
      <div className="flex flex-row gap-6">
        {!comment['report_details']['archive'] && <button onClick={handleArchiveReport} className={navButton}>Archive</button>}
